  ...
  return (
    <div className="flex flex-col gap-5 max-w-[1800px] mx-auto w-full">
      ...
      {conceptLabel && (
        <div className="flex items-center justify-between gap-3 bg-red-50 border border-red-200 rounded-xl px-4 py-2.5">
          ...
          <button
            ...
            className="text-[11px] font-bold text-[#DC2626] hover:text-[#991B1B] flex items-center gap-1"
          >
            <X size={12} /> Temizle
          </button>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <button onClick={() => setShowFilters(!showFilters)} className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold border transition-all ${showFilters || hasActiveFilters ? 'bg-red-50 text-[#DC2626] border-red-200 shadow-sm' : 'bg-white text-slate-600 border-slate-200 hover:border-red-200'}`}>
          <SlidersHorizontal size={14} /> {t('common.filter')} {hasActiveFilters && <span className="bg-[#DC2626] text-white text-[9px] w-4 h-4 rounded-full flex items-center justify-center">!</span>}
        </button>
        {showPromo && (
        ...
        <button onClick={() => setFilter('inStockOnly', !filters.inStockOnly)} className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold border transition-all ${filters.inStockOnly ? 'bg-red-50 text-[#DC2626] border-red-200' : 'bg-white text-slate-600 border-slate-200 hover:border-red-200'}`}>
        ...
        <div className="flex bg-white rounded-xl border border-slate-200 p-0.5 ml-auto">
          {([['grid', Grid3X3], ['list', List], ['table', Columns3]] as const).map(([mode, Icon]) => (
            <button key={mode} onClick={() => setViewMode(mode as any)} className={`p-2 rounded-lg transition-all ${viewMode === mode ? 'bg-[#DC2626] text-white shadow-sm' : 'text-slate-400 hover:text-[#DC2626]'}`}><Icon size={16} /></button>
          ))}
        </div>

        {viewMode === 'table' && (
          <div className="relative">
            <button onClick={() => setShowColPicker(!showColPicker)} className="px-3 py-2 rounded-xl text-xs font-bold bg-red-50 text-[#DC2626] border border-red-200 hover:bg-red-100 transition-all">
              ...
              <div className="absolute right-0 top-full mt-2 w-80 max-h-96 overflow-y-auto bg-white rounded-xl shadow-2xl border border-slate-200 z-50 p-3">
                <div className="flex justify-between items-center mb-3">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => setVisibleCols(ALL_COLUMNS.map(c => c.key))} className="text-[10px] text-[#DC2626] font-bold hover:underline">{t('common.all')}</button>
      ...
      {!isLoading && !error && products.length === 0 && (
        <EmptyState
          ...
          action={(filters.search || filters.family || filters.promoOnly || filters.newOnly || filters.inStockOnly) && (
            <button onClick={resetFilters} className="px-4 py-2 bg-[#DC2626] text-white text-sm font-bold rounded-lg hover:bg-[#991B1B] transition-colors">
        ...
        <div className="flex items-center justify-between py-2">
          ...
          <div className="flex items-center gap-1">
            ...
            {pageNumbers.map((page) => (
              <button key={page} onClick={() => setPage(page)} className={`w-8 h-8 flex items-center justify-center rounded text-xs font-bold transition-colors ${page === currentPage ? 'bg-[#DC2626] text-white shadow-sm' : 'text-on-surface-variant hover:bg-surface-container-high'}`}>{page}</button>
            ))}
            ...
            <div className="relative">
              ...
              {has3DModel(detailItem.id) && (
                <button
                  ...
                  className="absolute bottom-3 right-3 flex items-center gap-1.5 px-3 py-2 rounded-lg bg-[#DC2626] text-white text-xs font-black uppercase tracking-wider shadow-lg hover:bg-[#991B1B] transition-all"
                >
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                ...
                <div className="bg-surface-container-highest rounded-lg p-3">
                  ...
                  <div className="flex flex-wrap gap-1">
                    ...
                    {detailItem.has_accessories && <span className="px-2 py-0.5 bg-red-100 text-[#DC2626] text-[10px] font-bold rounded">{t('diamond.hasAccessories')}</span>}
