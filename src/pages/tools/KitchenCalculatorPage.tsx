  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              {(Object.keys(SEGMENT_FACTORS) as Segment[]).map((s) => (
                <button
                  ...
                  className={`px-3 py-2 rounded-lg text-sm font-medium border transition ${
                    segment === s
                      ? 'bg-[#7B1F26] text-white border-[#7B1F26]'
                      : 'bg-white text-slate-700 border-slate-300 hover:border-[#7B1F26]'
                  }`}
                >
        ...
        <div className="bg-gradient-to-br from-[#7B1F26] to-[#5A1219] text-white rounded-2xl p-6 space-y-4">
          ...
          <div className="pt-4 border-t border-white/20">
            <a
              href="/design"
              className="block w-full text-center py-3 bg-white text-[#7B1F26] rounded-xl font-bold hover:bg-red-50 transition"
            >
  ...
  return (
    <div>
      <label className="flex justify-between text-sm font-semibold text-slate-700 mb-2">
        <span>{label}</span>
        <span className="text-[#7B1F26]">{value.toLocaleString('tr-TR')}</span>
      </label>
      <input
        type="range"
        min={min}
        ...
        className="w-full accent-[#7B1F26]"
      />
    </div>
  );
}
